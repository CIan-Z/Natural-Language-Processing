...
def dm01():
    # 1. 定义待分词的文本内容
    content = '看到我们出现他们提早快跑，把这圈子水平拉高，从不倚老卖老，被我一招带跑，几秒败到，击倒菜鸟，力道太吊，技巧代表'
...
def dm02():
    # 1. 定义待分词的文本内容
    content = '扫平所有障碍 is hero come back，看到我的出现就像神灵驾到，现在换我上台保持最佳状态，我们总是轰炸就像人形炸药'

    # 2. 使用jieba进行 全模式分词 -> cut_all = True，会发现全模式下的分词比精确模式多很多
    # result1: 生成器对象, 好处: 节省内存, 只能遍历一次
    ...
    # 4. 如果要列表怎么办?, 即: [词1, 词2, 词3...]
    # 思路1: 直接把上述的生成器 -> 转成 列表
    list1 = list(result1)  #list1 为空，是因为 result1 是一个生成器，它只能被完整遍历一次。
    print(f'list1: {list1}')
...
def dm03():
    ...
    # 2. 使用jieba进行 搜索引擎模式分词 -> cut_for_search() todo：在精确模式基础上，对长词再次进行切分，提高召回率
    # result1: 生成器对象, 好处: 节省内存, 只能遍历一次
