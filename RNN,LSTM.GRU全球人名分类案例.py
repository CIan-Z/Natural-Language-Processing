...
晚上练习顺序:
    ...
    还有时间和精力, 继续往下做:
        ...
        5. 把我说的可以优化的地方改进下.
            ...
            4. .......
    在监督学习中，把数据集拆成数据（特征）和标签（目标）是最核心的步骤，原因是模型需要通过“题目”和“标准答案”来学习事物之间的映射关系。
    学习目标明确：模型拿到数据（输入），计算出预测结果，然后用标签（真实答案）对比，计算误差（损失）。
    误差反向传播：正是因为有“真实答案”做参照，优化器才能知道模型的参数应该往哪个方向调整，让预测越来越准。
    评估效果：训练完后，用另一批没见过的数据（只给题目，不给答案）让模型预测，再把预测结果和隐藏的标签比对，才能知道模型到底学会了没有，准确率是多少。
"""
...
class NameClassDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # todo：解释： max(index, 0)：取 index 和 0 中的较大值，保证结果不低于 0。min(..., self.sample_len - 1)：再取这个临时结果和 self.sample_len - 1 中的较小值，保证最终结果不超过上限。
        # 2. 按照索引获取原始样本 和 标签.todo：本质：“通过一个已经被‘修正过’的索引，去数据库里拿对应的记录”。
        x = self.my_list_x[index]       # 例如: Ding      ->  (4, 57)代表4个字母，57个可能出现的形式
        ...
        # 4. 国家数据转换为 张量.将字符串形式的类别标签 y 转换为其在类别列表 categories 中的索引（整数），再包装成一个 torch.long 类型的张量。
        tensor_y = torch.tensor(categories.index(y), dtype=torch.long)
        # 单个样本的标签常用标量张量存储，比如 tensor(3)，它代表类别索引 3。
        # 多个样本的标签堆叠在一起，就变成一维张量（向量），如 tensor([0,1,2])。DataLoader 设置 batch_size=4，它一次性从数据集中抓取了 4 个样本，堆叠就是把一个个单独的数字（标量索引），根据批次内的顺序排列好，固定成一个长方形的数组（一维向量），方便计算机一次性计算所有样本的误差。
        # 5. 返回结果
...
def get_dataloader():
    # 1. 读取数据文件, 获取: 样本(人名)列表 和 标签(国家名)列表.将文件中的两列内容分别存入两个独立的列表 my_list_x（样本/姓名）和 my_list_y（标签/国籍）。
    ...
    #它的形状通常是 (batch_size, seq_len, input_size)（即 [1, 序列长度, 特征维度]），而不是 RNN 默认需要的 (seq_len, batchsize, input_size)。
...
class My_RNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        #经常一条一条地处理（batch_size=1），但传进来的张量可能是二维的 [seq_len, input_size]。为了复用同一个 forward 并满足 RNN 的格式检查，就在这里统一补上 batch 维度。
        #PyTorch 中 nn.RNN（以及 LSTM、GRU）要求输入形状必须是 三维
        input = input.unsqueeze(1)

        # 2.2 通过RNN计算.
        # output: 所有时间步的隐藏状态   hidden: 最后1个时间步的隐藏状态
        output, hn = self.rnn(input, hidden)
        # 2.3 提取最后1个时间步的隐藏状态.此时我们只关心模型读完整个序列后的“总体理解”，所以只取最后一个时间步的输出（output[-1]）或最终的隐藏状态 hn。
        tmp_output = output[-1]             # 形状为: [batch_size, hidden_size]
        # 因为模型需要为每一个预测动作准备一个“独立”的输出特征，用来和正确答案一一对应计算损失。
        # 多对一（N to 1）任务：模型的目标是“看完一串字母后，只猜一个国籍”。损失函数计算时，只问模型：“你最后认为他是哪国人？”
        # 多对多（N to N）任务：模型的目标是“每看一个字母，就要猜出这个字母接下来应该是什么属性”。
        # 语言模型（如歌词生成）中：输入 [w1, w2, w3]，目标输出 [w2, w3, w4]。模型在位置 1 输出特征的目的是为了预测位置 1 的下一个词（即 w2），所以位置 1 的特征必须有对应的预测头，否则无法计算位置 1 的损失。
        # 2.4 通过全连接层, 获取输出.
        tmp_output = self.linear(tmp_output)
        # 全连接层（self.linear）：只负责做线性变换，把 RNN 输出的 hidden_size 维向量变成 output_size 维的原始分数（logits）。它里面只有权重和偏置，没有任何非线性压缩。
        # Softmax（self.softmax，用的是 LogSoftmax）：只负责把「分数」映射成「概率分布」或者「对数概率分布」，让所有类别的输出值介于 0~1 之间（或对数空间），总和为 1。
        # 2.5 数据通过激活函数, 映射到概率分布, 并返回.
        return self.softmax(tmp_output), hn
        #全连接层通过一个可学习的权重矩阵，把输入特征向量（比如 RNN 隐藏状态）映射到一个新的向量，其维度等于类别总数，里面的每个数代表模型对相应类别的“原始打分”，之后再经过 softmax 才变成概率。
        #输入是一个名字的字符序列，RNN 读完最后一步输出一个 hidden_size 的特征向量。
        #全连接层把这个特征向量和权重矩阵相乘，得到一个长度为 57 的向量（假如你有 57 个字母类）。这 57 个数就是模型对“下一个字母是 a”、“下一个字母是 b”……的倾向性打分。
        #最后通过 softmax 把打分转成 57 个概率值，加起来等于 1。这时我们就可以说“模型觉得下一个字母是 z 的概率是 0.05 之类”。
...
class My_LSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, n_layers=1):
        ...
        # todo 优化： self.dropout = nn.Dropout(0.3)
        # 1.7 定义全连接层, 将RNN的隐藏状态转换成输出.
        self.linear = nn.Linear(self.hidden_size, self.output_size)
        ...
        # 大白话解释: 多分类交叉熵损失函数CrossEntropyLoss(新版写法) = NLLLoss损失函数 + LogSoftmax(dim=-1)  旧版写法
        self.softmax = nn.LogSoftmax(dim=-1)        # 优化2: 如果用CrossEntropyLoss损失函数, 这行代码可以省略不写.
    #todo优化：重点：损失函数的使用，两种档案。方案A：保留 LogSoftmax → 把损失函数改成 nn.NLLLoss()。实现了交叉熵损失函数的功能。方案B：删除 self.softmax，直接返回 tmp_output，损失函数继续用 nn.CrossEntropyLoss()，直接实现交叉熵损失函数
    ...
    def forward(self, input, hidden, c):
        ...
        # 2.3 提取最后1个时间步的隐藏状态.
        tmp_output = output[-1]             # 形状为: [batch_size, hidden_size]
        # todo ：优化：last_out = self.dropout(output[-1])
        # 2.4 通过全连接层, 获取输出.
        tmp_output = self.linear(tmp_output)
...
def dm_test_myrnn():
    ...
    input = torch.randn(6, 57) #todo：这一步就能看出来为什么前面用 input = input.unsqueeze(1)，根本就没用迭代器，是直接传入的二维数据    # liru: ouyang 欧阳
    ...
    # 4. 测试一次性输入完整的一个样本(序列数据) 注意：output的hidden_size被处理成了18，本来是128
    output, hn = my_rnn(input, h0)

    # 5. 打印结果.
    print(f'输出的形状: {output.shape}, 输出的内容: {output}')         # [1, 18] ，因为这里是N对1，所以只要最后一个状态，所以seq_len被去掉了
...
def dm_test_rnn_lstm_gru():
    ...
    # 3. 创建数据集对象
    name_class_dataset = NameClassDataset(my_list_x, my_list_y)
    # 4. 创建数据加载器.todo：这里的batch_size只能为1，因为名字的长度可能都不一样
    my_dataloader = DataLoader(name_class_dataset, batch_size=1, shuffle=True)
    # 5. 模型初始化.
    ...
    for i, (x, y) in enumerate(my_dataloader):
        ...
        # 7.3 前向传播.forward 期望的输入形状是 [seq_len, input_size]（二维），然后在内部通过 unsqueeze(1) 补上 batch 维度变成 [seq_len, 1, input_size]。
...
def train_rnn():
    ...
    for epoch in range(epochs):     # epoch: 第几轮
        ...
        for i, (x, y) in enumerate(tqdm(train_dataloader)):     # 优化点3: 这里加入进度条.
            ...
            # 4.7 计算当前样本预测准确率 从模型输出的预测概率（或logits）中，找出概率最大的那个类别的索引，并将其从 PyTorch 张量转换为 Python 原生整数。
...
def dm_predict_rnn(x):
    # 1. 定义遍历, 记录模型相关参数.（输入特征维度）（隐藏层维度）（输出类别数）
    ...
    with torch.no_grad():
        ...
        # 5.2 从预测结果中, 获取前3个最大的元素.在学术或工程评估中，Top-3 Accuracy 是一个重要指标。它衡量模型把正确答案排在前 3 位的比例。
        # 参1(k):   取前3个最大的元素.
...
def dm_predict_lstm(x):
    ...
    with torch.no_grad():
        #如果 LSTM 是多层的，或者后面还有时间步（如在生成任务中），你就必须把 c 传递下去，否则 LSTM 会错误地认为初始状态是全零，丢失了上下文。
        h0, c0 = my_lstm.init_hidden()
        output, hidden,c = my_lstm(x_tensor,h0, c0 )
        topv, topi = output.topk(3, 1, True)
        print(f'rnn(待预测文本): {x}')
        for i in range(3):
            value = topv[0][i].item()           # 概率值 -> Python的标量
...
if __name__ == '__main__':
    ...
    # 7. 测试: 模型预测.
    dm_predict_rnn('Gordusenko')
    dm_predict_lstm('Gordusenko')
    dm_predict_gru('Gordusenko')