...
def normalizeString(s):
    ...
    s = re.sub('([.!?])', r' \1', s)#外层的圆括号 () 将这个字符类包裹起来，使其成为一个捕获组。匹配到的标点会被捕捉并保留下来，以便在替换字符串 r' \1' 中使用 \1 来引用它。
    # print(f's: {s}')
...
def my_getdata():
    ...
    with open(data_path, 'r', encoding='utf-8') as src_f:
        ...
        '''
        函数调用 normalizeString(s)
        对每个 s 调用归一化函数，通常做小写转换、去标点前后的空格、添加开始/结束标记等工作。
        例如：normalizeString("Hi.") → "hi ."
        '''
        ...
        # 4. 初始化英语词汇表
        # todo 4.1 创建单词到索引的字典映射, 预定义特殊字符SOS(句子开始), EOS(句子结束)的索引为: 0, 1
        ...
        for pair in my_pairs:       # pair的数据格式: ['英语句子', '法语句子'], 例如: ['i m not doing it anymore .', 'je ne vais plus le faire .']
            # 6.2 对每个英语句子处理, 将句子按照 空格 切割成单词列表, 然后遍历, 获取到每个单词.将 pair[0] 这个字符串按空格分割，然后逐个取出每个词进行处理。
            for word in pair[0].split(' '):
                # 6.3 检查单词是否在词汇表中, 如果不存在, 就给改单词分配1个新索引, 新索引值 = 当前词汇表的大小.
                if word not in english_word2index:
                    english_word2index[word] = english_word_n # 变为例如{'fuck': 2, 'shit': 3}
                    # 细节: 词汇表大小计数器 +1
                    english_word_n += 1
        ...
        # 7. 构建反向映射表(索引到单词的映射)
        # 7.1 英语词汇表的反向映射.
        english_index2word = {v: k for k, v in english_word2index.items()}  # 将“词→索引”的映射字典，翻转成“索引→词”的映射字典。
        # 7.2 法语词汇表的反向映射.
...
class MyPairsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 5. 返回处理后的数据(英文张量, 法语张量)，每个句子都已经被转换成了对应单词索引的张量
        return tensor_x, tensor_y
    #这两个方法会在你使用 DataLoader 加载数据时自动触发，不需要手动调用。
    #__len__：当你创建 DataLoader 并访问其长度（如 len(data_loader)）或在进行迭代计算总步数时被调用，用于返回数据集的总样本数，以确定 epoch 和 batch 的划分。
    #__getitem__：在 DataLoader 迭代过程中，每次需要获取一个或多个样本时，它会根据索引逐个调用 __getitem__ 来获取具体样本（经过各种批处理和 shuffle 等操作后）。简言之，训练循环中的 for batch in data_loader 就会在底层不断触发 __getitem__。
...
def get_dataloader():
    ...
    # for i, (x, y) in enumerate(my_dataloader):
        ...
        # 6. 测试结束 -> 只显示一批数据即可.
        # break
#转成索引形式（每个单词都成了索引），是因为word embedding只能把索引转换成词向量，预测的时候还能根据索引找到原始的词
    # 7. 返回数据加载器对象.
...
def dm_test_decoder():
    ...
    for i, (x, y) in enumerate(my_dataloader):
        ...
        h0 = my_encoder_gru.init_hidden()
        # 4.3.2 (编码器)前向传播.
        encoder_output_c, hidden = my_encoder_gru(x, h0)#todo ：其实就是得到的c，给解码器使用
...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, output_size, hidden_size, dropout_p=0.1, max_length=MAX_LENGTH):
        ...
        '''
        max_length 是源序列的最大长度（或者可能就是要关注的位置个数）。线性层的输出是一个长度为 max_length 的向量，向量中的每一个数字，就是对应位置（第 1 个词、第 2 个词……）的原始匹配分。
        也就是说，这个线性层一次性为所有可能的位置打出了分数，不需要循环一个个算。这等价于：
        $$\text{score}_j = \text{Linear}_{\text{第}j\text{个输出单元}}([\text{查询}; \text{键的特征}])$$
        '''
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        # [1, 1] -> [1, 1, 256]                             todo：当前步的输入：来自上一时刻生成的词
        ...
        # step3: 应用softmax()层, 映射为: [1, 10] -> [1, 10]  todo：构造查询（Query）—— 拼接当前词嵌入和上一步隐藏状态
        attn_weights = F.softmax(self.attn(torch.cat((embedded[0], hidden[0]), 1)), dim=1)
        # 4. 计算注意力上下文.                                 todo： 加权求和得到上下文向量 C
        attn_applied = torch.bmm(attn_weights.unsqueeze(0), encoder_outputs.unsqueeze(0))

        # 5.注意力融合层.
        output = torch.cat((embedded[0], attn_applied[0]), 1)       # [1, 1, 512]  todo：拼接当前词向量和C，embedded[0] 就是当前时间步你喂给解码器的那个词（通常是上一步生成的词，或者训练时的目标语言词）所对应的词向量。
...
def train_seq2seq():
    ...
    for epoch_idx in range(1, epochs + 1):
        ...
        for item, (x, y) in enumerate(tqdm(my_dataloader), start=1):
            # tqdm(my_dataloader)：用 tqdm 包装数据加载器，每取出一个 batch 都会更新进度条，让你直观看到当前训练进度、处理速度等信息。
            # enumerate(..., start=1)：对包装后的迭代器进行枚举，产生 (序号, 批次数据) 的元组，序号从 1 开始（而不是默认的 0），方便在日志里记录“正在处理第几个 batch”。
...
def evaluate_seq2seq(x, my_encoder_rnn, my_attn_decoder_rnn):   # 英语句子, 编码器, 解码器(带注意力机制)
    ...
    with torch.no_grad():
        ...
        for idx in range(MAX_LENGTH):
            ...
            '''
            output_y（当前步的输出概率分布）：用于确定当前预测的词（取最大值作为下一步的输入），或者在训练时与目标词比较来计算损失。这是解码器对外最直接的结果。
            decode_hidden（更新后的隐藏状态）：作为下一时间步的隐藏状态输入。因为解码是逐词串行进行的，每一步都要把这一步产生的 hidden 传给下一步，以保持语境连续性。
            attn_weights（注意力权重分布）：不参与控制流程，但用于可视化分析。它可以被记录下来，画成热力图，直观展示当前生成词对源句中各个词的关注程度。在纯推理时如果不需可视化，可以忽略，但仍然返回以保持接口统一。
            '''
            ...
            topv, topi = output_y.topk(1)
            '''
            output_y 是解码器当前时间步的输出，形状为 [1, output_size]（例如 [1, 4345]），里面每个值是对应法语词汇的对数概率（因为用了 LogSoftmax）。
            output_y.topk(1) 的作用是：取出这一行里数值最大的那个元素（即概率最高的词），并返回两个值：
            topv：这个最大值的具体数值（对数概率）。
            topi：这个最大值对应的索引，也就是预测出的法语单词在词汇表中的编号。
            '''
...
# decoder_attentions[:idx + 1] 是为了只保留真正执行过的解码步数所对应的注意力权重，去掉后面多余的、填充为 0 的行。
# decoder_attentions 初始化为 (MAX_LENGTH, MAX_LENGTH) 的全零张量。
# 在解码循环中，每一步（索引 idx）都会把该步的注意力权重存入 decoder_attentions[idx]。
# 当循环因为预测到 EOS 而 break 时，idx 就是最后一步的索引（假设在第 k 步 break，那么 idx = k），此时实际上已经执行了 k+1 个解码步（从 0 到 k）。
# 因此切片 [: idx + 1] 正好取出前 k+1 行的有效注意力记录，丢弃后面从未被赋值的所有零行。这样返回的矩阵维度就是 (实际步数, MAX_LENGTH)，能准确对应每一步的注意力分布，避免多余的全零行干扰后续的可视化或分析。
