...
def dm02_chain():
    ...
    print(f'result: {result}')
    # chain 的行为非常简单：它接收多个可迭代对象，依次把它们的元素取出来串联在一起。
    #不加 *：chain 把整个 map 迭代器当做一个整体，输出的每个元素是分词后的词列表。此时就没有另一个迭代器与他相连，所以会原封不动的输出内容
    # 加 *：把 map 迭代器中的每个词列表解开成独立参数，chain 把它们串联成一个扁平的词序列。
